--
-- show_versions.sql - show banner based on current and update version strings
--
DISCONNECT
...
--
DECLARE
    v_new_core_version VARCHAR2(100) := '&&CORE_VERSION';
    v_new_client_version VARCHAR2(100) := '&&CLIENT_VERSION';
    v_core_version VARCHAR2(100);
    v_client_version VARCHAR2(100);
...
BEGIN
    ...
    -- core bundle
    --
    IF (LENGTH(v_new_core_version) IS NOT NULL)
    AND (LENGTH(v_new_client_version) IS NULL)
    THEN
        dbms_output.put_line( '.');
        dbms_output.put_line( '. This bundle will update Dragon to::');
        dbms_output.put_line( '.');
        dbms_output.put_line( '. Core version:   '||v_new_core_version);
        dbms_output.put_line( '.');
    --
    -- client bundle
    --
    ELSIF (LENGTH(v_new_core_version) IS NULL)
    AND (LENGTH(v_new_client_version) IS NOT NULL)
    THEN
        dbms_output.put_line( '.');
        dbms_output.put_line( '. This bundle will update Dragon to::');
        dbms_output.put_line( '.');
        dbms_output.put_line( '. Client version: '||v_new_client_version);
        dbms_output.put_line( '.');
    --
    -- core and client bundle
    --
    ELSIF (LENGTH(v_new_core_version) IS NOT NULL)
    AND (LENGTH(v_new_client_version) IS NOT NULL)
    THEN
        dbms_output.put_line( '.');
        dbms_output.put_line( '. This bundle will update Dragon to::');
