...
#Path
FilePath="/home/oracle/scripts/UTICAREPORT/CPUTICAF/DATAMART/"
...
@$ScriptLoc/insert_dr_data_reconcile_metadata_datamart.sql;
@$ScriptLoc/vw_dr_data_reconcile_dm_history.sql;
@$ScriptLoc/create_dr_data_reconcile_datamart_table.sql;
@$ScriptLoc/data_validation_process_datamart.sql;
@$ScriptLoc/insert_dm_data_reconcile_history.sql;
@$ScriptLoc/generate_insert_dm_data_reconcile_history.sql;
...
echo "<br/><h3>Object Comparison results between Object Tables vs Dragon Datamart</h3>" >> $HtmlFile
...
then
	...
	(
	 ...
	 cat $HtmlFile
	) | mutt -s "[DataReconcile][DatamartVsObject] [$ENV]" $EMAIL_LIST -e "set content_type=text/html; my_hdr From:$EMAIL_FROM" < $HtmlFile 
