...
--Process to run data validation using the dr_data_reconcile_object table
declare
  v_rpt_date date default trunc(sysdate);
  v_schema_prefix varchar2(30) := '%OSSCMEHA_%'; --pass this value as a parameter
  v_sql varchar2(10000);
  v_sql_frame varchar2(10000);
  v_bi_dataset varchar2(30);
  v_dragon_object_count number;
  v_bi_object_count number;
...
begin
	v_sql_frame := '
				select
					...
					missing_rows,
					additional_rows,
					round((missing_rows/(case when dragon_object_count = 0 then 1 else dragon_object_count end)) * 100 , 2) difference
				from
				(Select
					''%DEST_TABLE%'' bi_dataset,
					sum((case when ro.%PK_COLUMN% is not null then 1 else 0 end)) dragon_object_count,
					sum((case when dt.%PK_COLUMN% is not null then 1 else 0 end)) bi_object_count,
					--sum((case when dt.%PK_COLUMN% is not null and ro.%PK_COLUMN% is not null then 1 else 0 end)) total_matched,
	...
	loop
	   ...
	   then
		   merge into
			    dr_data_reconcile_history rh
			using (select
					  v_rpt_date report_date,
					  v_bi_dataset table_name, 
					  v_dragon_object_count dragon_object_count,
					  v_bi_object_count bi_object_count,
		  ...
		  update
		  set
			  dragon_object_count = stg.dragon_object_count,
			  bi_object_count = stg.bi_object_count,
